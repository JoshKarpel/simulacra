...
if __name__ == '__main__':
    with cp.utils.Logger(stdout_level = logging.DEBUG, file_logs = True, file_dir = OUT_DIR, file_level = logging.DEBUG) as logger:
        ...
        optics = [disp.BK7(name = '8 lenses', length = 8 * 3 * mm),
                  disp.FS(name = 'cavity', length = 1 * inch),
        ...
        #                                                                              "D:\GitHubProjects\compy\dispersion\data\\tisapph_spectrum2.txt", total_power = 150 * mW,
        #                                                                              plot_fit = True, target_dir = OUT_DIR)

        wavelengths, power = np.loadtxt("D:\GitHubProjects\compy\dispersion\data\\tisapph_spectrum2.txt", delimiter = ',', unpack = True, skiprows = 1)
        wavelengths = nm * wavelengths
        power = 1 * mW * (10 ** (power / 10))
        amplitudes = np.sqrt(power)

        spec = disp.ContinuousAmplitudeSpectrumSpecification('TiSapph', c / wavelengths, amplitudes, optics)
