...
if dispersion:
    OUT_DIR = os.path.join(OUT_DIR, 'dispersion')
else:
    ...
    with cp.utils.LogManager(stdout_level = logging.DEBUG, file_logs = False, file_dir = OUT_DIR, file_level = logging.DEBUG) as logger:
        ...
        if dispersion:
            optics = [
                ...
                disp.ModulateBeam(90 * THz, upshift_efficiency = 1e-4, downshift_efficiency = 0),
        ...
        for method in methods:
            ...
            sim.plot_electric_field_vs_time(target_dir = OUT_DIR, x_lower_lim = -200 * fsec, x_upper_lim = 200 * fsec, find_center = False, name_postfix = '_center_before_modulation')
            ...
            # sim.plot_electric_field_vs_time(target_dir = OUT_DIR)
            sim.plot_electric_field_vs_time(target_dir = OUT_DIR, x_lower_lim = -200 * fsec, x_upper_lim = 200 * fsec, find_center = False, name_postfix = '_center_after_modulation')
            sim.plot_power_vs_frequency(target_dir = OUT_DIR, x_scale = 'THz')
            sim.plot_power_vs_wavelength(target_dir = OUT_DIR, x_scale = 'nm')
