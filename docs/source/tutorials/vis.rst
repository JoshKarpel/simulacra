...
That code will produce a file ``y_vs_x.png`` in the current working directory,
containing the image shown below:

.. image:: ../figs/y_vs_x.*
   :align: center

We can probably improve this a little.
``x`` should be measured in radians, so let's tell Simulacra that.
We can also add some labels, which we can use LaTeX syntax inside.
