    ...
    with figure_manager as fm:
        ...
        y_lower_limit, y_upper_limit = vutils.set_axis_limits_and_scale(
            ...
            *(
                ...
                for y_func, y_kwargs in zip(y_funcs, y_func_kwargs)
                for t in t_data
            ),
        ...
        if y_label is not None:
            y_label = ax.set_ylabel(
                r"{}".format(y_label) + y_unit_label, fontsize=font_size_axis_labels
            )
        ...
        if x_unit == "rad":
            ticks, labels = vutils.get_pi_ticks_and_labels(x_lower_limit, x_upper_limit)
            ax.set_xticks(ticks)
            ax.set_xticklabels(labels)
        if y_unit == "rad":
            ticks, labels = vutils.get_pi_ticks_and_labels(y_lower_limit, y_upper_limit)
            ax.set_yticks(ticks)
            ax.set_yticklabels(labels)

        if x_extra_ticks is not None and x_extra_tick_labels is not None:
            ax.set_xticks(
                list(ax.get_xticks()) + list(np.array(x_extra_ticks) / x_unit_value)
        ...
        if y_extra_ticks is not None and y_extra_tick_labels is not None:
            ax.set_yticks(
                list(ax.get_yticks()) + list(np.array(y_extra_ticks) / y_unit_value)
            )  # append the extra tick labels, scaled appropriately
        ...
        lines = []
        for y_func, y_kwargs, lab, kw in itertools.zip_longest(
            y_funcs, y_func_kwargs, line_labels, line_kwargs
        ):
            if y_kwargs is None:
                y_kwargs = {}
            ...
            lines.append(
                plt.plot(
                    x_data / x_unit_value,
                    np.array(y_func(x_data, t_data[0], **y_kwargs)) / y_unit_value,
                    label=lab,
                    **kw,
                    animated=True,
                )[0]
        ...
        if len(line_labels) > 0:
            if not legend_on_right:
                legend = ax.legend(
                    loc="upper right", fontsize=font_size_legend, **legend_kwargs
                )
            if legend_on_right:
                legend = ax.legend(
        ...
        t_text = plt.figtext(0.7, 0.05, t_str, **t_text_kwargs, animated=True)
        ...
        with utils.SubprocessManager(cmd, **FFMPEG_PROCESS_KWARGS) as ffmpeg:
            for t in t_iter:
                ...
                for line, y_func, y_kwargs in zip(lines, y_funcs, y_func_kwargs):
                    line.set_ydata(
                        np.array(y_func(x_data, t, **y_kwargs)) / y_unit_value
                    )
                    fig.draw_artist(line)

                # update and redraw t strings
                t_text.set_text(
                    t_fmt_string.format(u.uround(t, t_unit, digits=3), t_unit_label)
                )
                fig.draw_artist(t_text)

                for artist in itertools.chain(
...
def xyzt_plot(
    ...
    **kwargs,
    ...
    if figure_manager is None:
        figure_manager = figman.FigureManager(name, save=False, **kwargs)
    with figure_manager as fm:
        fig = fm.fig
        ax = fig.add_axes([0.15, 0.15, 0.75, 0.7])
        ...
        if title is not None:
            title = ax.set_title(r"{}".format(title), fontsize=font_size_title)
            title.set_y(vutils.DEFAULT_TITLE_OFFSET)  # move title up a little
        if x_label is not None:
            x_label = ax.set_xlabel(
                r"{}".format(x_label) + x_unit_label, fontsize=font_size_axis_labels
            )
        ...
        with utils.SubprocessManager(cmd, **FFMPEG_PROCESS_KWARGS) as ffmpeg:
            for t in t_iter:
                ...
                if shading == vutils.ColormapShader.FLAT:
                    z = z[:-1, :-1]
...
class Animator(abc.ABC):
    ...
    def initialize(self, sim):
        ...
        try:
            os.remove(
                self.file_path
            )  # ffmpeg complains if you try to overwrite an existing file, so remove it first
        except FileNotFoundError:
            pass

        ideal_frame_count = self.length * self.fps
        self.decimation = int(
            self.sim.available_animation_frames / ideal_frame_count
        )  # determine ideal decimation from number of available frames in the simulation
    ...
    def _redraw_frame(self):
        ...
        plt.set_cmap(
            self.colormap
        )  # make sure the colormap is correct, in case other figures have been created somewhere

        self.fig.canvas.restore_region(
            self.background
        )  # copy the static background back onto the figure
        ...
        for rd in itertools.chain(
            self.redraw, *(ax.redraw for ax in self.axis_managers)
        ):
            self.fig.draw_artist(rd)

        self.fig.canvas.blit(
            self.fig.bbox
        )  # blit the canvas, finalizing all of the draw_artists
    ...
    def send_frame_to_ffmpeg(self):
        ...
        logger.debug(
            "{} sending frame to ffpmeg from {} {}".format(
                self, self.sim.__class__.__name__, self.sim.name
            )
        )
        ...
        logger.debug(
            "{} sent frame to ffpmeg from {} {}".format(
                self, self.sim.__class__.__name__, self.sim.name
            )
        )
