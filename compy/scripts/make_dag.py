...
def get_submit_file_paths(*job_names, jobs_dir = None):
    ...
    return (os.path.abspath(os.path.join(jobs_dir, job_dir, 'submit_job.sub')) for job_dir in os.listdir(jobs_dir) if job_dir in job_names)


def write_dag_files(job_names, submit_file_paths, max_jobs_idle = 5000, overwrite = False):
    dag_str = ['CONFIG dag_config']
    dag_str += [f'JOB {job_name} {submit_file_path}' for job_name, submit_file_path in zip(job_names, submit_file_paths)]

    name = f"dag-{'-'.join(job_names)}"
    dag_name = name + '.dag'
...
if __name__ == '__main__':
    ...
    with cp.utils.Logger('compy', 'ionization', stdout_level = 31 - ((args.verbosity + 1) * 10)) as logger:
        ...
        dag_file_name = write_dag_files(args.job_names, submit_file_paths, max_jobs_idle = args.max_jobs, overwrite = args.overwrite)
